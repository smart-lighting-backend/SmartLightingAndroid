...
```json
[
  { "deviceId": "SL_009", "name": "设备9", "area": "A区", "status": 1 },
  { "deviceId": "SL_010", "name": "设备10", "area": "A区", "status": 1 }
]
```
...
**告警类型 (type)：** OFFLINE（离线）、FAULT（故障）、HEALTH_LOW（健康分过低）

**告警级别 (level)：** CRITICAL（紧急）、MAJOR（严重）、WARNING（警告）、MINOR（提示）、INFO（提示）

**告警状态 (status)：** ACTIVE（待处理）、ACKNOWLEDGED（处理中）、RECOVERED（已解决）
...
```
GET /api/vision-events/page?page=1&size=20&deviceId=SL_001&eventType=行人检测
```
...
| page | int | 否 | 页码，默认 1 |
| size | int | 否 | 每页条数，默认 20 |
| deviceId | String | 否 | 设备编号（筛选） |
| eventType | String | 否 | 事件类型：行人检测 / 车辆通行 / 异常停车 / 危险场景 |
...
|------|------|------|
| deviceId | String | 设备编号 |
| eventType | String | 事件类型（行人检测/车辆通行/异常停车/危险场景） |
| confidence | BigDecimal | 识别置信度（0~100） |
| snapshotRef | String | 抓拍快照引用路径/URL |
| occurredAt | String | 事件发生时间 ISO 格式 |

### 11.2 指定设备视觉事件

```
GET /api/vision-events/device/{deviceId}?page=1&size=10
```

返回指定设备的分页视觉事件列表，按事件发生时间倒序排列。
...
```
GET /api/voice-events/page?page=1&size=20&deviceId=SL_001&type=警告
```
...
|------|------|------|
| deviceId | String | 设备编号 |
| type | String | 类型（播报/广播/警告） |
| content | String | 语音内容文本 |
| source | String | 来源（自动/手动/系统） |
| occurredAt | String | 事件发生时间 ISO 格式 |

### 12.2 指定设备语音事件

```
GET /api/voice-events/device/{deviceId}?page=1&size=10
```

返回指定设备的分页语音事件列表，按事件发生时间倒序排列。

---

## 十三、融合感知面板

一次性聚合设备的最新遥测、视觉事件、语音事件、告警和健康评分，适合设备详情页的综合监控展示。
...
| telemetry | Object / null | 最新遥测快照（同 5.1 data 对象） |
| latestVision | Object / null | 最新视觉事件 |
| latestVoice | Object / null | 最新语音事件 |
| recentAlarms | Array | 最近 3 条告警 |

**latestVision 对象：**

| 字段 | 类型 | 说明 |
|------|------|------|
| eventType | String | 事件类型（已做中文归一化） |
| confidence | BigDecimal | 识别置信度 |
| snapshotRef | String | 抓拍快照引用 |
...
| type | String | 告警类型 |
| level | String | 告警级别 |
| startAt | String | 告警开始时间 |
| recoverAt | String | 恢复时间（可为 null） |
...
> **结论：当前后端 API 完整覆盖移动端第一阶段所有需求 + 视觉/语音事件扩展模块，共 38 个接口，无缺失。** 策略设置（strategysettingsscreen.png）是原型图中的额外页面，不在四个目标模块范围内，如需实现需额外对接 PolicyController。
---
